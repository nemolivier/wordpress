var ExpressCurateSEOControlWidget = (function ($) {
    var isSetup = false,
        $metaDescription = $('textarea[name=expresscurate_description]'),
        $addKeywordsWrap = $('.addKeywords'),
    ...
    function insertKeywordInWidget(keywords) {
    ...
    function showKeywords() {
        var $contentWrap = $('#content'),
            ...
            titleText = $('#titlediv').find('input[name=post_title]').val(),
            contentWords = wordsInText(content.toLowerCase()),
            keywords = ExpressCurateKeywordUtils.justText($('#expresscurate_defined_tags')).toLowerCase().split(', '),
            keywordHtml = '';

        if (keywords.length > 0 && keywords[0] !== '') {
            $.each(keywords, function (index, value) {
                var data = {},
                    myRegExp = new RegExp('((^|\\s|>|))(' + value + ')(?=[^>]*(<|$))(?=(&nbsp;|\\s|,|\\.|:|!|\\?|\'|\"|\\;|.?<|$))', 'gmi'),
                    numOccurencesContent = (content.match(myRegExp)) ? content.match(myRegExp).length : 0,
                    numOccurencesTitle = titleText.match(myRegExp) ? titleText.match(myRegExp).length : 0;
                $.extend(data, {
                    'inTitle': (numOccurencesTitle > 0 ? "yes" : "no"),
                ...
                if (data.inContent < 3) {
                    data.color = 'blue';
                } else if (data.inContent >= 3 && data.inContent <= 5) {
                    data.color = 'green';
                } else if (data.inContent > 5) {
                    data.color = 'red';
                }
                if (data.inContent == 0) {
                    data.inContent = Math.round(parseInt(data.inContent));
                }
                keywordHtml += ExpressCurateUtils.getTemplate('SEOControlCenter', data);
            });
            $('.expresscurate_background_wrap').remove();
            $('.addKeywords').before(keywordHtml);
        }
    }
    ...
    function setupKeywords() {
        if (typeof(tinyMCE) === "object" && typeof(tinyMCE.execCommand) === "function" && $('.expresscurate_widget').length > 0) {
            var checkEditor = setTimeout(function check() {
                if ($('#content').length) {
                    clearTimeout(checkEditor);
                    showKeywords();
    ...
    function insertKeywordInPost(keyword) {
        ExpressCurateUtils.track('/post/content/keyword/insert');
    ...
    function setupEvents() {
        /*add keywords*/
        $('.expresscurate_widget_wrapper .addKeywords input').on("keyup", function (e) {
            if (e.keyCode === 38 || e.keyCode === 40 || e.keyCode === 27) {
                e.preventDefault();
                return;
            }
            var $list = $addKeywordsWrap.find('.suggestion li');
            if (e.keyCode === 13) {
                $list.remove();
                insertKeywordInWidget(ExpressCurateKeywordUtils.multipleKeywords($addInput, $keywordWrap));
                $addKeywordsWrap.find('.suggestion li').hide();
            } else {
        ...
        $('.expresscurate_widget').on("click", '.suggestion li', function () {
            var text = $(this).text();
            $addInput.val(text);
            insertKeywordInWidget(ExpressCurateKeywordUtils.multipleKeywords($addInput, $keywordWrap));
            $('.suggestion li').remove();
        });
        ...
        $('#title').on("keyup", function (e) {
            if (e.altKey && e.keyCode === 75) {
                ...
                if ($('#expresscurate_widget').length && keyword.length > 3) {
                    keyword = keyword.replace(/<[^>]+>[^<]*<[^>]+>|<[^\/]+\/>/ig, "");
                    $addInput.val(keyword);
                    insertKeywordInWidget(ExpressCurateKeywordUtils.multipleKeywords($addInput, $keywordWrap));
                }
            }
        });

        $widgetWraper.keydown(function (event) {
            if (event.keyCode === 13) {
        ...
        $('.expresscurate_widget_wrapper .addKeywords span span').on('click', function () {
            insertKeywordInWidget(ExpressCurateKeywordUtils.multipleKeywords($addInput, $keywordWrap));
        });

        /*delete keywords*/
        $widgetWraper.on('click touchend', '.expresscurate_background_wrap .close', function () {
            ExpressCurateKeywordUtils.close($(this).parent().find('.statisticsTitle').text(), $(this).parents('.expresscurate_background_wrap'));
        });
        ...
        $('.description textarea').on('keyup focus', function () {
            if ($widgetWraper.length) {
                var maxVal = 156,
                    ...
                    count = $descriptionTextarea.val().length,
                    val,
                    $textarea = $('.description textarea'),
                    keywords = [],
                    metaDesc = $descriptionTextarea.val(),
                    includedKeywordsCount = 0,
                    defKeywords = $('#expresscurate_defined_tags').val();
                ...
                $('.usedKeywordsCount').replaceWith('<p class="usedKeywordsCount"><span class="bold">' + includedKeywordsCount + '</span>' + ' / ' + keywords.length + '</p>');
                /**/
                if (count > maxVal) {
        ...
        $('.description, .description p').click(function () {
            if ($widgetWraper.length) {
                $('.description  p , .description .hint').removeClass('expresscurate_displayNone');
                $description.addClass('active');
                $descriptionContainer.removeClass('textareaBorder');
                $metaDescription.focus();

                ExpressCurateUtils.track('/post/seo-control-center/description/edit');
            }
        });

        $widgetWraper.click(function () {
            if ($widgetWraper.length) {
                $descriptionContainer.addClass('textareaBorder');
        ...
        $(document).click(function (e) {
            if ($('.expresscurate_widget').length > 0 && !$(e.target).parents('#expresscurate').is('div')) {
                $descriptionContainer.addClass('textareaBorder');
                $description.find('p').add($description.find('.hint')).addClass('expresscurate_displayNone');
                $description.removeClass('active');
            }
            // hide suggestion list if clicked outside
            if ($(e.target).is('.addKeywords .suggestion') || $(e.target).closest('.addKeywords .suggestion li').length) {
            } else {
                $addKeywordsWrap.find('.suggestion li').hide();
            }
        });
        ...
        // keyboard navigation
        ExpressCurateKeywordUtils.suggestionsKeyboardNav($('#expresscurate_widget').find(' .addKeywords input'));

        /*refresh keywords*/
        $('.expresscurate_widget_wrapper label .rotate').click(function () {
            var $el = $(this).addClass('rotated');
            showKeywords();
            setTimeout(function () {
        ...
        /*mark keyword*/
        $('.expresscurate_widget_wrapper .mark').on('touchend', function (e) {
            if (e.target === $('.expresscurate_widget_wrapper .mark')) {
                ExpressCurateKeywordUtils.markEditorKeywords();
            }
        });
        $('#expresscurate_widget_wrapper').on('click', '.mark', function () {
            ExpressCurateKeywordUtils.markEditorKeywords();
        });
        ...
        /*Advanced SEO*/
        $('#expresscurate_advanced_seo_nofollow, #expresscurate_advanced_seo_noindex').on('change', function () {
            var $this = $(this),
        ...
        $('.expresscurate_moveToAdvanced').on('click', function () {
            ExpressCurateUtils.animateScroll($("#expresscurate_advanced_seo"));
        });
...
ExpressCurateSEOControlWidget.setup();