...
class ExpressCurate_Sitemap
{
    ...
    public function submitToGoogle()
    {
        ...
        if ($submitToWebmastersStatus) {
            ...
            if ($response) {
                $sitemapDomain = get_option('expresscurate_sitemap_domain', '');
                $response = $googleAuth->submitSitemap(strlen($sitemapDomain) > 2 ? $sitemapDomain : get_site_url(), get_site_url() . '/sitemap.xml');
            } else {
